# EveTradingbot changelog archive — 2026-08-18 to 2026-08-21

Document role: **historical evidence.** These are the `CHANGELOG.md` entries from the
planning commit through the fill models, moved here verbatim on 2026-09-04 when the
changelog was bounded (JumpStarter retrofit). Nothing below was edited in the move; the
heading levels are as they were. The current inventory and the recent entries are in
[`../CHANGELOG.md`](../CHANGELOG.md).

Read one entry to answer one specific question. Never load this file as context.
