...
| Liquidity floor | **DERIVED FROM THE CENSUS, 2026-08-20** (this row replaces the pre-census estimate of 100M ISK / 30 orders, per §8 Phase 1). Rule, stated before the measurement: *fewest types while capturing ≥95% of median daily ISK turnover*. Result: **median 30d `isk_value` ≥ 500,000,000 ISK**, **median `order_count` ≥ 0** — admitting **2,654 types** carrying **95.1%** of median daily turnover, from 19,152 Forge-active types. |
| ⚠ Floor caveat — `order_count` fell out | The derived floor's `order_count` component is **zero**, because the stated rule optimizes for *fewest types at 95% of turnover* and a pure ISK-turnover floor achieves that. The rule was followed exactly; the consequence is that the **anti-wash-trade protection §3.6 and §4 wanted from `order_count` is not active in the derived floor**. `volume` alone being gameable by one wash trade was the reason `order_count` was in the floor at all. This is a real gap between a rule and its intent, and it is **not** to be fixed by quietly re-deriving until the answer looks right: either the rule gains an explicit `order_count ≥ k` constraint as a plan-level edit with a stated reason, or the config floor keeps its own `min_median_order_count` above the derived one. Until the operator decides, `config.toml`'s `min_median_order_count` remains the effective guard. |
