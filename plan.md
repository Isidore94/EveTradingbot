...
| Decision | Value |
|---|---|
| Control set | **Amended 2026-09-04 (§17 D-36, JumpStarter retrofit):** `CLAUDE.md` (agent operating rules; `AGENTS.md` is a generated byte-identical copy for Codex — one source, one sync, never hand-edited), `plan.md` (this file — roadmap + contracts), `CHANGELOG.md` (searchable implemented inventory + bounded recent changes; older entries archived under `docs/`), `CURRENT_CHECKPOINT.md` (the "Active state at a glance" block + active item + the live-validation checklist), `WISHLIST.md` (ideas, never authorized work), `docs/README.md` (classifies every Markdown file). Agent roles under `.claude/agents/` and `.codex/agents/`, packets under `.claude/packets/`, the evidence behind every rule in `docs/INTERNALS.md`. *Superseded wording, 2026-08-18:* "`CLAUDE.md`, `plan.md`, `CHANGELOG.md`, `CURRENT_CHECKPOINT.md`. No AGENTS.md copy — one file, one truth" — the repo had in fact carried an identical `AGENTS.md` since 2026-08-25. |
