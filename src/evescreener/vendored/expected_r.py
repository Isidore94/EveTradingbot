...
# Upstream: scripts/master_avwap_lib/expected_r.py
    ...
    *,
    ...
    if confident_wr is None:
        ...
        if profit_factor is not None:
            ...
            evidence += (
                min(pf, float(cfg["pf_cap"])) / float(cfg["pf_cap"]) * float(cfg["pf_weight"])
            )
...
def calibrate_prior_anchors(
    ...
    *,
    ...
    bins = _quantile_bins(
        pairs,
        int(cal.get("num_bins", 4) or 4),
        int(cal.get("min_bin_samples", 6) or 6),
    )
    if len(bins) < 2:
        return {
            "anchors": default_anchors,
